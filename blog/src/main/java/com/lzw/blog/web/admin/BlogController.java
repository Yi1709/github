...
public class BlogController {
	...
	public String blogs(@PageableDefault(size = 2, sort = {"updateTime"}, direction = Sort.Direction.DESC) Pageable pageable, BlogQuery blog,
		...
		model.addAttribute("page", this.blogService.listBolg(pageable, blog));
		return LIST;
	}

	@PostMapping("/blogs/search")
	public String search(@PageableDefault(size = 2, sort = {"updateTime"}, direction = Sort.Direction.DESC) Pageable pageable, BlogQuery blog,
	                     Model model) {
		model.addAttribute("page", this.blogService.listBolg(pageable, blog));
	...
	public String post(Blog blog, RedirectAttributes attributes, HttpSession session) {
		...
		Blog blog1 = this.blogService.saveBlog(blog);
		if (blog1 == null) {
	...
	public String delete(@PathVariable Long id, RedirectAttributes attributes) {
		this.commentService.deleteByBlogId(id);
		this.blogService.deleteBlog(id);
		attributes.addFlashAttribute("message", "Success,删除成功!");
		return REDIRECT_LIST;
	}
}
